...
void test_min_max() {
  ...
  auto [min1, max1] = OrderStatistics::find_min_max(arr1);
  std::cout << "数组: ";
  for (int num : arr1)
    std::cout << num << " ";
  std::cout << std::endl;
  ...
  std::vector<int> arr2;
  try {
    auto [min2, max2] = OrderStatistics::find_min_max(arr2);
...
void test_randomized_select() {
  ...
  for (int i = 1; i <= arr1.size(); ++i) {
    ...
    int result = OrderStatistics::find_ith_smallest_randomized(arr_copy, i);
  ...
  int result_min = OrderStatistics::find_ith_smallest_randomized(arr2, 1);
  std::cout << "单元素数组第1小: " << result_min << " (正确)" << std::endl;
  std::cout << std::endl;
}

void test_linear_time_select() {
  ...
  for (int i = 1; i <= arr1.size(); ++i) {
    std::vector<int> arr_copy = arr1;
    int result = OrderStatistics::find_ith_smallest(arr_copy, i);
  ...
  double median = OrderStatistics::find_median(arr2);
  ...
  double median_even = OrderStatistics::find_median(arr3);
...
void test_algorithm_comparison() {
  ...
  for (int i = 1; i <= test_data.size(); ++i) {
    ...
    int result1 = OrderStatistics::find_ith_smallest(data_copy1, i);
    int result2 = OrderStatistics::find_ith_smallest_randomized(data_copy2, i);
...
void test_edge_cases() {
  ...
  try {
    ...
    OrderStatistics::find_ith_smallest(empty, 1);
  ...
  try {
    ...
    OrderStatistics::find_ith_smallest(data, 0);
  ...
  try {
    std::vector<int> data = {1, 2, 3};
    OrderStatistics::find_ith_smallest(data, 4);
  ...
  int result = OrderStatistics::find_ith_smallest(duplicates, 5);
