...
void performance_test(const std::vector<T>& arr, 
    ...
    std::cout << test_name << " - 排序验证: " 
              << (LinearTimeSortGeneric::is_sorted(result) ? "通过" : "失败") << std::endl;
}
 ...
 */
template<typename T>
void performance_test_counting(const std::vector<T>& arr, 
                               std::vector<T> (*sort_func)(const std::vector<T>&, T, T), 
                               T min_val, T max_val,
                               const std::string& test_name) {
    auto start = std::chrono::high_resolution_clock::now();
...
int main() {
    ...
    std::cout << "验证结果: " << (CountingSort<int>::is_sorted(result1) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "验证结果: " << (CountingSort<long>::is_sorted(result2) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "验证结果: " << (RadixSort<int>::is_sorted(result3) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "验证结果: " << (BucketSort<double>::is_sorted(result4) ? "通过" : "失败") << std::endl;
    
    // 测试5：桶排序通用版本（浮点数）
    std::cout << std::endl << "测试5：桶排序通用版本（浮点数）" << std::endl;
    std::vector<float> test5 = {15.3f, 12.7f, 18.9f, 11.2f, 19.5f, 13.8f, 16.1f, 14.6f, 17.4f};
    LinearTimeSortGeneric::print_array(test5, "排序前");
    
    std::vector<float> result5 = BucketSort<float>::sort_general(test5, 11.0f, 20.0f);
    LinearTimeSortGeneric::print_array(result5, "排序后");
    std::cout << "验证结果: " << (BucketSort<float>::is_sorted(result5) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "空数组测试: " << (CountingSort<int>::is_sorted(empty_result) ? "通过" : "失败") << std::endl;
    
    // 单元素数组
    std::vector<int> single = {42};
    std::vector<int> single_result = CountingSort<int>::sort(single, 42);
    std::cout << "单元素数组测试: " << (CountingSort<int>::is_sorted(single_result) ? "通过" : "失败") << std::endl;
    
    // 已排序数组
    std::vector<int> sorted = {1, 2, 3, 4, 5};
    std::vector<int> sorted_result = CountingSort<int>::sort(sorted, 5);
    std::cout << "已排序数组测试: " << (CountingSort<int>::is_sorted(sorted_result) ? "通过" : "失败") << std::endl;
    ...
    std::vector<int> counting_result = CountingSort<int>::sort(counting_test, 100);
    ...
    std::cout << "计数排序 - 排序验证: " << (CountingSort<int>::is_sorted(counting_result) ? "通过" : "失败") << std::endl;
    ...
    // 桶排序性能测试
    std::vector<double> bucket_test = generate_random_double_array<double>(1000, 0.0, 1.0);
    performance_test(bucket_test, BucketSort<double>::sort, "桶排序");
    
    // 测试8：与std::sort的性能对比
    std::cout << std::endl << "测试8：与std::sort的性能对比（数组大小: 1000）" << std::endl;
    
    std::vector<int> std_sort_test = generate_random_int_array<int>(1000, 0, 1000);
    ...
    std::cout << "std::sort - 时间: " << duration_std.count() << " 微秒" << std::endl;
    std::cout << "std::sort - 排序验证: " << (LinearTimeSortGeneric::is_sorted(std_result) ? "通过" : "失败") << std::endl;
    
    // 测试9：不同类型数据测试
    std::cout << std::endl << "测试9：不同类型数据测试" << std::endl;
    
    // 长整型计数排序
    std::vector<long> long_test = generate_random_int_array<long>(500, 1000L, 10000L);
    performance_test_counting(long_test, CountingSort<long>::sort_general, 1000L, 10000L, "长整型计数排序");
    
    // 浮点数桶排序
    std::vector<float> float_test = generate_random_double_array<float>(500, 50.0f, 150.0f);
    performance_test_counting(float_test, BucketSort<float>::sort_general, 50.0f, 150.0f, "浮点数桶排序");
    ...
    // CountingSort<std::string>::sort(string_test, 0); // 编译错误：计数排序仅适用于整数类型
    // RadixSort<std::string>::sort(string_test);       // 编译错误：基数排序仅适用于整数类型
    // BucketSort<std::string>::sort(string_test);       // 编译错误：桶排序仅适用于浮点数类型
    */
