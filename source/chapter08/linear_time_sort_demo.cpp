 ...
 * @return 随机数组
 */
std::vector<int> generate_random_int_array(int size, int min_val = 0,
                                           int max_val = 100) {
 ...
 * @return 随机数组
 */
std::vector<double> generate_random_double_array(int size, double min_val = 0.0,
                                                 double max_val = 1.0) {
...
void performance_test(const std::vector<T> &arr,
  ...
  std::cout << test_name << " - 排序验证: "
            << (LinearTimeSort::is_sorted(result) ? "通过" : "失败")
            << std::endl;
}
...
void performance_test_counting(
    ...
    int min_val, int max_val, const std::string &test_name) {
  auto start = std::chrono::high_resolution_clock::now();
...
int main() {
  ...
  std::cout << "验证结果: "
            << (LinearTimeSort::is_sorted(result1) ? "通过" : "失败")
            << std::endl;
  ...
  std::vector<int> result2 =
      LinearTimeSort::counting_sort_general(test2, 11, 19);
  LinearTimeSort::print_array(result2, "排序后");
  std::cout << "验证结果: "
            << (LinearTimeSort::is_sorted(result2) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "验证结果: "
            << (LinearTimeSort::is_sorted(result3) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "验证结果: "
            << (LinearTimeSort::is_sorted(result4) ? "通过" : "失败")
            << std::endl;

  // 测试5：桶排序通用版本
  std::cout << std::endl << "测试5：桶排序通用版本" << std::endl;
  std::vector<double> test5 = {15.3, 12.7, 18.9, 11.2, 19.5,
                               13.8, 16.1, 14.6, 17.4};
  LinearTimeSort::print_array(test5, "排序前");

  std::vector<double> result5 =
      LinearTimeSort::bucket_sort_general(test5, 11.0, 20.0);
  LinearTimeSort::print_array(result5, "排序后");
  std::cout << "验证结果: "
            << (LinearTimeSort::is_sorted(result5) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "空数组测试: "
            << (LinearTimeSort::is_sorted(empty_result) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "单元素数组测试: "
            << (LinearTimeSort::is_sorted(single_result) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "已排序数组测试: "
            << (LinearTimeSort::is_sorted(sorted_result) ? "通过" : "失败")
            << std::endl;
  ...
  std::vector<int> counting_result =
      LinearTimeSort::counting_sort(counting_test, 100);
  ...
  std::cout << "计数排序 - 时间: " << duration_counting.count() << " 微秒"
            << std::endl;
  std::cout << "计数排序 - 排序验证: "
            << (LinearTimeSort::is_sorted(counting_result) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "std::sort - 排序验证: "
            << (LinearTimeSort::is_sorted(std_result) ? "通过" : "失败")
            << std::endl;
  ...
  for (int size : {100, 1000, 10000}) {
    ...
    if (size <= 10000) { // 避免过大范围
      ...
      auto start_counting = std::chrono::high_resolution_clock::now();
      std::vector<int> counting_result =
          LinearTimeSort::counting_sort(counting_data, 100);
      auto end_counting = std::chrono::high_resolution_clock::now();
      auto duration_counting =
          std::chrono::duration_cast<std::chrono::microseconds>(end_counting -
                                                                start_counting);
      std::cout << "计数排序 - 时间: " << duration_counting.count() << " 微秒"
                << std::endl;
      std::cout << "计数排序 - 排序验证: "
                << (LinearTimeSort::is_sorted(counting_result) ? "通过"
                                                               : "失败")
                << std::endl;
    }
    ...
    std::cout << "std::sort - 时间: " << duration.count() << " 微秒"
              << std::endl;
  }
  ...
  std::vector<Element> stability_test = {{2, 1}, {5, 2}, {3, 3}, {2, 4},
                                         {3, 5}, {2, 6}, {5, 7}};
