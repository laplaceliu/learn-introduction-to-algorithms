...
void testBinaryCounterAggregateAnalysis() {
  std::cout << "=== 17.1节 聚合分析：二进制计数器 ===" << std::endl;

  BinaryCounter counter(8);

  std::cout << "初始状态: " << counter.getBinaryString()
            << " (十进制: " << counter.getValue() << ")" << std::endl;
  ...
  for (int i = 1; i <= 16; ++i) {
    ...
    std::cout << std::setw(4) << i << std::setw(12) << counter.getBinaryString()
              << std::setw(10) << counter.getValue() << std::setw(8) << flips
              << std::setw(12) << counter.getTotalFlips() << std::endl;
  }

  std::cout << "\n聚合分析结果：" << std::endl;
  std::cout << "- 总操作次数: 16" << std::endl;
  std::cout << "- 总翻转次数: " << counter.getTotalFlips() << std::endl;
  std::cout << "- 平均每次操作翻转次数: " << std::fixed << std::setprecision(2)
            << (double)counter.getTotalFlips() / 16 << std::endl;
  std::cout << "- 理论分析：每个位大约翻转n/2^i次，总翻转次数为O(n)"
            << std::endl;
  std::cout << std::endl;
}

void testMultipopStackAccountingMethod() {
  ...
  std::cout << std::setw(12) << "PUSH(1)" << std::setw(8) << stack.size()
            << std::setw(10) << stack.getCredit() << std::setw(15) << "2"
            << std::endl;
  ...
  std::cout << std::setw(12) << "MULTIPOP(2)" << std::setw(8) << stack.size()
            << std::setw(10) << stack.getCredit() << std::setw(15) << "0"
            << std::endl;
...
void testDynamicTablePotentialMethod() {
  ...
  for (int i = 1; i <= 9; ++i) {
    ...
    int potential = table.getPotential();
    int amortized_cost = analyzer.recordOperation(actual_cost, potential);

    std::cout << std::setw(6) << "INSERT" << std::setw(8) << table.getNum()
              << std::setw(8) << table.getSize() << std::setw(10) << actual_cost
  ...
  std::cout << "- 总实际代价: " << analyzer.getTotalActualCost() << std::endl;
  std::cout << "- 最终势能: " << analyzer.getCurrentPotential() << std::endl;
  std::cout << "- 总摊还代价: "
            << analyzer.getTotalActualCost() + analyzer.getCurrentPotential()
            << std::endl;
  std::cout << "- 平均摊还代价: " << std::fixed << std::setprecision(2)
            << (double)(analyzer.getTotalActualCost() +
                        analyzer.getCurrentPotential()) /
                   analyzer.getOperationCount()
            << std::endl;
  std::cout << std::endl;
}

void testTableExpansionContraction() {
  ...
  for (int i = 1; i <= 17; ++i) {
    table.insert(i);
    std::cout << "插入 " << i << ": 元素数=" << table.getNum()
              << ", 表大小=" << table.getSize() << std::endl;
  }

  std::cout << "\n删除操作导致表收缩：" << std::endl;
  for (int i = 1; i <= 9; ++i) {
    table.deleteLast();
    std::cout << "删除 " << i << ": 元素数=" << table.getNum()
              << ", 表大小=" << table.getSize() << std::endl;
  }
...
void testAmortizedAnalysisComparison() {
...
int main() {
  ...
  testBinaryCounterAggregateAnalysis();
  testMultipopStackAccountingMethod();
  testDynamicTablePotentialMethod();
  testTableExpansionContraction();
  testAmortizedAnalysisComparison();
