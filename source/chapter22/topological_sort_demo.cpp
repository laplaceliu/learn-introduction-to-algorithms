...
void test_dfs_topological_sort() {
  ...
  for (size_t i = 0; i < dfs_order.size(); i++) {
    for (size_t j = i + 1; j < dfs_order.size(); j++) {
      ...
      if (std::find(neighbors.begin(), neighbors.end(), dfs_order[i]) !=
          neighbors.end()) {
...
void test_kahn_topological_sort() {
  ...
  for (size_t i = 0; i < kahn_order.size(); i++) {
    for (size_t j = i + 1; j < kahn_order.size(); j++) {
      ...
      if (std::find(neighbors.begin(), neighbors.end(), kahn_order[i]) !=
          neighbors.end()) {
...
void test_cycle_detection() {
  ...
  std::cout << "图是否为有向无环图(DAG): " << (is_dag ? "是" : "否")
            << std::endl;
...
void test_algorithm_correctness() {
  ...
  std::cout << "\n正确性验证:" << std::endl;
  std::cout << "  DFS算法结果是否有效: " << (dfs_valid ? "是" : "否")
            << std::endl;
  std::cout << "  Kahn算法结果是否有效: " << (kahn_valid ? "是" : "否")
            << std::endl;
