...
void test_classic_example() {
  ...
  for (const auto &key : keys) {
    ...
    if (node != nullptr) {
      std::cout << "  找到 " << key << " (概率=" << std::fixed
                << std::setprecision(3) << node->probability << ")"
                << std::endl;
    } else {
...
void test_edge_cases() {
  ...
  std::cout << "  期望搜索成本: " << std::fixed << std::setprecision(3)
            << obst2.get_expected_cost() << std::endl;
...
void test_different_probability_patterns() {
  ...
  std::cout << "  期望搜索成本: " << std::fixed << std::setprecision(3)
            << obst1.get_expected_cost() << std::endl;
  std::cout << "  树高度: " << obst1.get_height() << std::endl;
  ...
  std::cout << "  期望搜索成本: " << std::fixed << std::setprecision(3)
            << obst3.get_expected_cost() << std::endl;
...
void test_algorithm_correctness() {
  ...
  for (const auto &test_case : test_cases) {
    ...
    std::cout << "  期望成本: " << std::fixed << std::setprecision(3)
              << test_case.expected_cost << std::endl;
    std::cout << "  实际成本: " << std::fixed << std::setprecision(3)
              << actual_cost << std::endl;
  ...
  std::cout << "正确性验证结果: " << passed << "/" << total << " 通过"
            << std::endl;
  if (passed == total) {
...
void test_comparison_with_normal_bst() {
  ...
  std::vector<double> probabilities = {0.05, 0.1, 0.15, 0.2,
                                       0.15, 0.1, 0.05, 0.1};
  ...
  for (size_t i = 0; i < keys.size(); i++) {
    ...
    normal_bst_cost_estimate +=
        probabilities[i] * (std::log(keys.size()) / std::log(2) + 1);
  }

  std::cout << "普通二叉搜索树（估算）:" << std::endl;
  std::cout << "  估算搜索成本: " << std::fixed << std::setprecision(3)
            << normal_bst_cost_estimate << std::endl;

  if (obst_cost < normal_bst_cost_estimate) {
    std::cout << "  最优BST比普通BST更优: 正确" << std::endl;
    std::cout << "  优化比例: " << std::fixed << std::setprecision(1)
              << ((normal_bst_cost_estimate - obst_cost) /
                  normal_bst_cost_estimate * 100)
              << "%" << std::endl;
  } else {
