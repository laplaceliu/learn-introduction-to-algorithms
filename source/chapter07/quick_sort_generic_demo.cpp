...
int main() {
  ...
  std::cout << "验证结果: "
            << (QuickSortGeneric<int>::is_sorted(test1) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "验证结果: "
            << (QuickSortGeneric<double>::is_sorted(test2) ? "通过" : "失败")
            << std::endl;

  // 测试3：字符串类型快速排序
  std::cout << std::endl << "测试3：字符串类型快速排序" << std::endl;
  std::vector<std::string> test3 = {"banana", "apple", "cherry", "date",
                                    "elderberry"};
  ...
  std::cout << "验证结果: "
            << (QuickSortGeneric<std::string>::is_sorted(test3) ? "通过"
                                                                : "失败")
            << std::endl;
  ...
  QuickSortGeneric<int>::sort(test4,
                              QuickSortGeneric<int>::descending_comparator());
  ...
  std::cout << "降序排序验证: " << (descending_sorted ? "通过" : "失败")
            << std::endl;

  // 测试5：自定义比较函数
  std::cout << std::endl
            << "测试5：自定义比较函数（按绝对值排序）" << std::endl;
  ...
  std::cout << "验证结果: "
            << (QuickSortGeneric<int>::is_sorted(test6) ? "通过" : "失败")
            << std::endl;
  ...
  auto duration_int = std::chrono::duration_cast<std::chrono::microseconds>(
      end_int - start_int);
  std::cout << "整数快速排序 - 时间: " << duration_int.count() << " 微秒"
            << std::endl;
  std::cout << "整数快速排序 - 排序验证: "
            << (QuickSortGeneric<int>::is_sorted(int_test) ? "通过" : "失败")
            << std::endl;

  // 浮点数性能测试
  std::vector<double> double_test =
      generate_random_array<double>(1000, 0.0, 1000.0);
  ...
  auto duration_double = std::chrono::duration_cast<std::chrono::microseconds>(
      end_double - start_double);
  std::cout << "浮点数快速排序 - 时间: " << duration_double.count() << " 微秒"
            << std::endl;
  std::cout << "浮点数快速排序 - 排序验证: "
            << (QuickSortGeneric<double>::is_sorted(double_test) ? "通过"
                                                                 : "失败")
            << std::endl;
  ...
  auto duration_random = std::chrono::duration_cast<std::chrono::microseconds>(
      end_random - start_random);
  std::cout << "随机化快速排序 - 时间: " << duration_random.count() << " 微秒"
            << std::endl;
  std::cout << "随机化快速排序 - 排序验证: "
            << (QuickSortGeneric<int>::is_sorted(randomized_test) ? "通过"
                                                                  : "失败")
            << std::endl;
  ...
  std::cout << "空数组测试: "
            << (QuickSortGeneric<int>::is_sorted(empty) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "单元素数组测试: "
            << (QuickSortGeneric<int>::is_sorted(single) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "已排序数组测试: "
            << (QuickSortGeneric<int>::is_sorted(sorted) ? "通过" : "失败")
            << std::endl;
  ...
  std::cout << "逆序数组测试: "
            << (QuickSortGeneric<int>::is_sorted(reverse) ? "通过" : "失败")
            << std::endl;
