 ...
 * @return 随机数组
 */
std::vector<int> generate_random_array(int size, int min_val = 1, int max_val = 100) {
...
void performance_test(std::vector<int> arr, 
    ...
    std::cout << test_name << " - 排序验证: " 
              << (QuickSort::is_sorted(arr) ? "通过" : "失败") << std::endl;
}
...
int main() {
    ...
    std::cout << "验证结果: " << (QuickSort::is_sorted(test1) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "验证结果: " << (QuickSort::is_sorted(test2) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "验证结果: " << (QuickSort::is_sorted(test3) ? "通过" : "失败") << std::endl;
    ...
    std::cout << "空数组测试: " << (QuickSort::is_sorted(empty) ? "通过" : "失败") << std::endl;
    
    // 单元素数组
    std::vector<int> single = {42};
    QuickSort::sort(single);
    std::cout << "单元素数组测试: " << (QuickSort::is_sorted(single) ? "通过" : "失败") << std::endl;
    
    // 已排序数组
    std::vector<int> sorted = {1, 2, 3, 4, 5};
    QuickSort::sort(sorted);
    std::cout << "已排序数组测试: " << (QuickSort::is_sorted(sorted) ? "通过" : "失败") << std::endl;
    
    // 逆序数组
    std::vector<int> reverse = {5, 4, 3, 2, 1};
    QuickSort::sort(reverse);
    std::cout << "逆序数组测试: " << (QuickSort::is_sorted(reverse) ? "通过" : "失败") << std::endl;
    ...
    // 测试6：大量重复元素的性能测试
    std::cout << std::endl << "测试6：大量重复元素性能测试（数组大小: 1000）" << std::endl;
    ...
    performance_test(dup_test2, QuickSort::three_way_sort, "三路快速排序（重复元素）");
    ...
    int pivot_index = QuickSort::partition(partition_demo, 0, partition_demo.size() - 1);
    QuickSort::print_array(partition_demo, "分区后");
    std::cout << "基准元素位置: " << pivot_index << " (值: " << partition_demo[pivot_index] << ")" << std::endl;
