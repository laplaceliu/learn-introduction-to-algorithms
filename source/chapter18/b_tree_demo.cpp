...
void test_basic_operations() {
    ...
    // 验证B树性质
    std::cout << "B树性质验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
    ...
    for (int key : {5, 15, 20, 25}) {
        ...
        std::cout << "搜索 " << key << ": " << (node != nullptr ? "找到" : "未找到") << std::endl;
    }
...
void test_delete_operations() {
    ...
    // 插入更多键值
    std::vector<int> keys = {1, 3, 7, 10, 11, 13, 14, 15, 18, 16, 19, 24, 25, 26, 21, 4, 5, 20, 22, 2, 17, 12, 6};
...
void test_edge_cases() {
    ...
    // 测试空树
    {
        ...
        std::cout << "空树搜索 5: " << (btree.search(5) != nullptr ? "找到" : "未找到") << std::endl;
        std::cout << "空树删除 5: " << (btree.remove(5) ? "成功" : "失败") << std::endl;
        std::cout << "空树验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
    }
    ...
    // 测试单节点树
    {
        ...
        std::cout << "单节点树验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
        
        // 删除唯一节点
        btree.remove(10);
        std::cout << "删除唯一节点后验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
    }
    ...
    // 测试重复插入和删除
    {
        ...
        // 删除不存在的键值
        std::cout << "删除不存在的键值 100: " << (btree.remove(100) ? "成功" : "失败") << std::endl;
        
        std::cout << "重复操作验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
    }
...
void test_different_degrees() {
    ...
    // 测试最小度数为2的B树
    {
        ...
        std::cout << "最小度数2验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
    ...
    // 测试最小度数为4的B树
    {
        ...
        std::cout << "最小度数4验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
        std::cout << "高度: " << btree.height() << std::endl;
    }
...
void test_performance() {
    ...
    std::cout << "插入100个键值后验证: " << (all_found ? "所有键值都存在" : "有键值丢失") << std::endl;
    std::cout << "B树性质验证: " << (btree.validate() ? "通过" : "失败") << std::endl;
    std::cout << "B树高度: " << btree.height() << std::endl;
...
void test_clrs_example() {
    ...
    std::vector<int> numeric_keys = {'G', 'M', 'P', 'X', 'A', 'C', 'D', 'E', 'J', 'K', 'N', 'O', 'R', 'S', 'T', 'U', 'V', 'Y', 'Z'};
    ...
    // 删除M（对应数字13）
    std::cout << "删除M(13): " << (btree.remove(13) ? "成功" : "失败") << std::endl;
