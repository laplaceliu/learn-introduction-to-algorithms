...
void test_basic_operations() {
  ...
  // 测试查找
  std::cout << "查找 19: " << (rbt.search(19) != nullptr ? "找到" : "未找到")
            << std::endl;
  std::cout << "查找 25: " << (rbt.search(25) != nullptr ? "找到" : "未找到")
            << std::endl;

  // 测试红黑树性质
  std::cout << "黑高度: " << rbt.black_height() << std::endl;
  std::cout << "是否有效红黑树: " << (rbt.is_valid() ? "是" : "否")
            << std::endl;
...
void test_delete_operations() {
  ...
  // 测试删除叶子节点
  std::cout << "删除叶子节点 1: " << (rbt.remove(1) ? "成功" : "失败")
            << std::endl;
  std::cout << "删除后中序遍历: ";
  ...
  // 测试删除有两个子节点的节点
  std::cout << "删除节点 11: " << (rbt.remove(11) ? "成功" : "失败")
            << std::endl;
  std::cout << "删除后中序遍历: ";
...
void test_edge_cases() {
  ...
  std::cout << "空树是否为空: " << (empty_rbt.empty() ? "是" : "否")
            << std::endl;
  ...
  std::cout << "是否有效红黑树: " << (single_rbt.is_valid() ? "是" : "否")
            << std::endl;
  ...
  std::cout << "是否有效红黑树: " << (duplicate_rbt.is_valid() ? "是" : "否")
            << std::endl;
  std::cout << std::endl;
}
...
void test_string_rbt() {
  ...
  // 测试查找
  std::cout << "查找 'banana': "
            << (string_rbt.search("banana") != nullptr ? "找到" : "未找到")
            << std::endl;
  std::cout << "查找 'fig': "
            << (string_rbt.search("fig") != nullptr ? "找到" : "未找到")
            << std::endl;

  std::cout << "是否有效红黑树: " << (string_rbt.is_valid() ? "是" : "否")
            << std::endl;
  std::cout << std::endl;
}
...
void test_complex_operations() {
  ...
  // 插入大量数据测试平衡性
  std::vector<int> data = {50, 30, 70, 20, 40, 60, 80, 10,
