...
void test_chained_hash_table() {
    ...
    std::cout << "搜索键32: " << (search_result ? *search_result : "未找到") << std::endl;
...
void test_hash_functions() {
    ...
    std::cout << "\n字符串散列:" << std::endl;
    std::vector<std::string> string_keys = {"hello", "world", "hash", "table", "algorithm"};
...
void test_open_addressing() {
    ...
    // 插入测试数据
    std::vector<std::pair<int, std::string>> test_data = {
        {10, "Alice"}, {22, "Bob"}, {31, "Charlie"}, {4, "David"},
        {15, "Eve"}, {28, "Frank"}, {17, "Grace"}, {88, "Henry"}, {59, "Ivy"}
    };
    ...
    std::cout << "搜索键22: " << (search_result ? *search_result : "未找到") << std::endl;
...
void test_perfect_hashing() {
    ...
    std::cout << "搜索不存在的键99: " << (not_found ? *not_found : "未找到") << std::endl;
...
void test_hash_distribution() {
    ...
    HashTableUtils::test_hash_distribution(
        [table_size](int key) { return HashFunctions::division_hash(key, table_size); },
        table_size, num_keys
    );
    
    std::cout << "\n乘法散列法分布:" << std::endl;
    HashTableUtils::test_hash_distribution(
        [table_size](int key) { return HashFunctions::multiplication_hash(key, table_size); },
        table_size, num_keys
    );
...
void test_performance_comparison() {
    ...
    for (int i = 0; i < num_operations; i++) {
        ...
        std::string op;
        switch (op_dis(gen)) {
            case 0: op = "insert"; break;
            case 1: op = "search"; break;
...
void test_clrs_examples() {
    ...
    std::vector<std::pair<int, std::string>> example_data = {
        {5, "卫星1"}, {28, "卫星2"}, {19, "卫星3"}, {15, "卫星4"},
        {20, "卫星5"}, {33, "卫星6"}, {12, "卫星7"}, {17, "卫星8"}, {10, "卫星9"}
    };
    ...
    std::vector<std::pair<int, std::string>> example_open_data = {
        {10, "A"}, {22, "B"}, {31, "C"}, {4, "D"}, {15, "E"}, {28, "F"}, {17, "G"}, {88, "H"}, {59, "I"}
    };
...
void test_edge_cases() {
    ...
    std::cout << "重复插入后搜索键1: " << (dup_result ? *dup_result : "未找到") << std::endl;
    
    // 测试删除不存在的键
    bool remove_result = dup_cht.remove(99);
    std::cout << "删除不存在的键99: " << (remove_result ? "成功" : "失败") << std::endl;
