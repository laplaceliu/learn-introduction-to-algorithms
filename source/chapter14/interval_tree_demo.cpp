...
void test_basic_operations() {
  ...
  // 测试插入（算法导论图14-4示例）
  std::cout << "插入区间: [16,21], [8,9], [25,30], [5,8], [15,23], [17,19], "
               "[26,26], [0,3], [6,10], [19,20]"
            << std::endl;
  ...
  // 测试精确搜索
  std::cout << "搜索区间 [15,23]: "
            << (it.search(Interval(15, 23)) != nullptr ? "找到" : "未找到")
            << std::endl;
  std::cout << "搜索区间 [10,15]: "
            << (it.search(Interval(10, 15)) != nullptr ? "找到" : "未找到")
            << std::endl;
...
void test_delete_operations() {
  ...
  // 测试删除叶子节点
  std::cout << "删除叶子节点 [26,26]: "
            << (it.remove(Interval(26, 26)) ? "成功" : "失败") << std::endl;
  std::cout << "删除后中序遍历: ";
  ...
  // 测试删除只有一个子节点的节点
  std::cout << "删除节点 [25,30]: "
            << (it.remove(Interval(25, 30)) ? "成功" : "失败") << std::endl;
  std::cout << "删除后中序遍历: ";
  ...
  // 测试删除有两个子节点的节点
  std::cout << "删除节点 [16,21]: "
            << (it.remove(Interval(16, 21)) ? "成功" : "失败") << std::endl;
  std::cout << "删除后中序遍历: ";
...
void test_edge_cases() {
  ...
  std::cout << "空树是否为空: " << (empty_it.empty() ? "是" : "否")
            << std::endl;
  ...
  std::cout << "搜索重叠区间 [15,25]: "
            << (result != nullptr ? "找到" : "未找到") << std::endl;

  result = single_it.interval_search(Interval(30, 40));
  std::cout << "搜索不重叠区间 [30,40]: "
            << (result != nullptr ? "找到" : "未找到") << std::endl;

  std::cout << "是否有效区间树: " << (single_it.is_valid() ? "是" : "否")
            << std::endl;
  std::cout << std::endl;
}
