...
struct MaxFlowResult {
  ...
  MaxFlowResult(int n)
      : max_flow_value(0), flow(n, std::vector<int>(n, 0)),
...
public:
  FlowNetwork(int n, int s, int t)
      : graph(true), capacities(n, std::vector<int>(n, 0)), source(s), sink(t) {
    // 添加节点
    for (int i = 0; i < n; i++) {
      graph.add_node(i);
    }
  ...
  // 获取容量
  int get_capacity(int u, int v) const { return capacities[u][v]; }
  ...
  void print_network() const {
    std::cout << "流网络 (源点: " << source << ", 汇点: " << sink
              << "):" << std::endl;
...
private:
  // BFS查找增广路径
  static bool bfs(const std::vector<std::vector<int>> &residual, int source,
                  int sink, std::vector<int> &parent) {
    int n = residual.size();
...
public:
  ...
  static int
  find_max_matching(const std::vector<std::vector<int>> &bipartite_graph,
  ...
  // 打印二分图匹配结果
  static void
  print_matching(const std::vector<std::vector<int>> &bipartite_graph,
                 int left_size, int right_size) {
    ...
    int max_matching =
        find_max_matching(bipartite_graph, left_size, right_size);
...
private:
  static void initialize_preflow(const std::vector<std::vector<int>> &capacity,
                                 ...
                                 std::vector<int> &excess, int source, int n) {
    height.assign(n, 0);
    excess.assign(n, 0);
      ...
      for (int u = 0; u < n; u++) {
        ...
        while (excess[u] > 0) {
          ...
          for (int v = 0; v < n && excess[u] > 0; v++) {
            if (result.capacity[u][v] - result.flow[u][v] > 0 &&
                height[u] == height[v] + 1) {
