...
// 区间树节点
template<typename T>
class IntervalTreeNode : public RBNode<T> {
public:
    Interval interval;
    ...
    void update_max() {
        ...
        if (this->left != nullptr) {
            auto left_node = std::static_pointer_cast<IntervalTreeNode<T>>(this->left);
            this->max = std::max(this->max, left_node->max);
        }
        if (this->right != nullptr) {
            auto right_node = std::static_pointer_cast<IntervalTreeNode<T>>(this->right);
...
private:
    ...
    void insert_fixup(std::shared_ptr<IntervalTreeNode<T>> z) {
        while (z->parent != nil && z->parent->color == Color::RED) {
            if (z->parent == z->parent->parent->left) {
                auto y = std::static_pointer_cast<IntervalTreeNode<T>>(z->parent->parent->right);
                ...
                if (y != nil && y->color == Color::RED) {
                    ...
                    z->parent->color = Color::BLACK;
                    z->parent->parent->color = Color::RED;
                    right_rotate(std::static_pointer_cast<IntervalTreeNode<T>>(z->parent->parent));
                }
            } else {
                auto y = std::static_pointer_cast<IntervalTreeNode<T>>(z->parent->parent->left);
                ...
                if (y != nil && y->color == Color::RED) {
                    ...
                    z->parent->color = Color::BLACK;
                    z->parent->parent->color = Color::RED;
                    left_rotate(std::static_pointer_cast<IntervalTreeNode<T>>(z->parent->parent));
                }
            }
        }
    ...
    // 移植操作（重写以更新max值）
    void transplant(std::shared_ptr<IntervalTreeNode<T>> u, std::shared_ptr<IntervalTreeNode<T>> v) {
        if (u->parent == nil) {
            root = v;
    ...
    // 查找最小节点
    std::shared_ptr<IntervalTreeNode<T>> minimum(std::shared_ptr<IntervalTreeNode<T>> node) const {
    ...
    void delete_fixup(std::shared_ptr<IntervalTreeNode<T>> x) {
        while (x != root && x->color == Color::BLACK) {
            if (x == x->parent->left) {
                auto w = std::static_pointer_cast<IntervalTreeNode<T>>(x->parent->right);
                ...
                if (w->color == Color::RED) {
                    w->color = Color::BLACK;
                    x->parent->color = Color::RED;
                    right_rotate(std::static_pointer_cast<IntervalTreeNode<T>>(x->parent));
    ...
    // 区间搜索辅助函数
    std::shared_ptr<IntervalTreeNode<T>> interval_search_helper(
        std::shared_ptr<IntervalTreeNode<T>> node, const Interval& interval) const {
        ...
        // 检查左子树
        if (node->left != nil) {
            auto left_node = std::static_pointer_cast<IntervalTreeNode<T>>(node->left);
        ...
        // 检查右子树
        if (node->right != nil) {
            auto right_node = std::static_pointer_cast<IntervalTreeNode<T>>(node->right);
    ...
    // 中序遍历辅助函数
    void inorder_helper(std::shared_ptr<IntervalTreeNode<T>> node, std::vector<Interval>& result) const {
        if (node != nil) {
            inorder_helper(std::static_pointer_cast<IntervalTreeNode<T>>(node->left), result);
            result.push_back(node->interval);
            inorder_helper(std::static_pointer_cast<IntervalTreeNode<T>>(node->right), result);
        }
    }
...
public:
    ...
    // 区间搜索：查找与给定区间重叠的区间
    std::shared_ptr<IntervalTreeNode<T>> interval_search(const Interval& interval) const {
    ...
    bool validate_max_values(std::shared_ptr<IntervalTreeNode<T>> node) const {
        ...
        return validate_max_values(std::static_pointer_cast<IntervalTreeNode<T>>(node->left)) &&
               validate_max_values(std::static_pointer_cast<IntervalTreeNode<T>>(node->right));
    }
