...
public:
     ...
     * @return 新创建的子节点
     */
    std::shared_ptr<TreeNode<T>> insert_child(std::shared_ptr<TreeNode<T>> parent, const T& value) {
        if (parent == nullptr) {
     ...
     */
    void preorder_traversal(std::shared_ptr<TreeNode<T>> node, std::vector<T>& result) const {
     ...
     * @param result 存储遍历结果的向量
     */
    void postorder_traversal(std::shared_ptr<TreeNode<T>> node, std::vector<T>& result) const {
        if (node == nullptr) return;
...
struct BinaryTreeNode {
    ...
    BinaryTreeNode(const T& value) : data(value), left(nullptr), right(nullptr), parent(nullptr) {}
...
public:
    ...
    std::shared_ptr<BinaryTreeNode<T>> get_root() const {
        return root;
    }
    ...
    std::shared_ptr<BinaryTreeNode<T>> insert_left(std::shared_ptr<BinaryTreeNode<T>> parent, const T& value) {
        if (parent == nullptr) {
            throw std::invalid_argument("父节点不能为空");
        }
    ...
    std::shared_ptr<BinaryTreeNode<T>> insert_right(std::shared_ptr<BinaryTreeNode<T>> parent, const T& value) {
        if (parent == nullptr) {
            throw std::invalid_argument("父节点不能为空");
        }
    ...
    void inorder_traversal(std::shared_ptr<BinaryTreeNode<T>> node, std::vector<T>& result) const {
        if (node == nullptr) return;
    ...
    void preorder_traversal(std::shared_ptr<BinaryTreeNode<T>> node, std::vector<T>& result) const {
        if (node == nullptr) return;
    ...
    void postorder_traversal(std::shared_ptr<BinaryTreeNode<T>> node, std::vector<T>& result) const {
        if (node == nullptr) return;
