...
template <typename T> class OBSTNode : public BSTNode<T> {
public:
  double probability; // 搜索概率

  OBSTNode(const T &k, double p = 0.0) : BSTNode<T>(k), probability(p) {}
...
// 最优二叉搜索树
template <typename T> class OptimalBinarySearchTree {
private:
...
public:
  OptimalBinarySearchTree(const std::vector<T> &k, const std::vector<double> &p)
    ...
    if (keys.size() != probabilities.size()) {
      throw std::invalid_argument(
          "Keys and probabilities must have the same size");
    }
    build_optimal_bst();
  }
  ...
  // 获取树的高度
  int get_height() const { return get_height_helper(root); }
  ...
  // 获取根节点
  std::shared_ptr<OBSTNode<T>> get_root() const { return root; }

  // 打印树结构
  void print_tree() const { print_tree_helper(root, 0, ""); }

private:
  // 构建最优BST的辅助函数
  std::shared_ptr<OBSTNode<T>>
  build_optimal_bst_helper(int i, int j,
    ...
    std::shared_ptr<OBSTNode<T>> node =
        std::make_shared<OBSTNode<T>>(keys[r - 1], probabilities[r - 1]);
  ...
  // 中序遍历辅助函数
  void inorder_helper(std::shared_ptr<OBSTNode<T>> node,
                      std::vector<T> &result) const {
    if (node != nullptr) {
      inorder_helper(std::dynamic_pointer_cast<OBSTNode<T>>(node->left),
                     result);
      result.push_back(node->key);
      inorder_helper(std::dynamic_pointer_cast<OBSTNode<T>>(node->right),
                     result);
    }
  }

  // 前序遍历辅助函数
  void preorder_helper(std::shared_ptr<OBSTNode<T>> node,
                       std::vector<T> &result) const {
    if (node != nullptr) {
      result.push_back(node->key);
      preorder_helper(std::dynamic_pointer_cast<OBSTNode<T>>(node->left),
                      result);
      preorder_helper(std::dynamic_pointer_cast<OBSTNode<T>>(node->right),
                      result);
    }
  }
  ...
  int get_height_helper(std::shared_ptr<OBSTNode<T>> node) const {
    ...
    return 1 +
           std::max(get_height_helper(
                        std::dynamic_pointer_cast<OBSTNode<T>>(node->left)),
                    get_height_helper(
                        std::dynamic_pointer_cast<OBSTNode<T>>(node->right)));
  }

  // 搜索辅助函数
  std::shared_ptr<BSTNode<T>> search_helper(std::shared_ptr<BSTNode<T>> node,
                                            const T &key) const {
  ...
  // 打印树结构辅助函数
  void print_tree_helper(std::shared_ptr<OBSTNode<T>> node, int level,
                         const std::string &prefix) const {
    if (node != nullptr) {
      print_tree_helper(std::dynamic_pointer_cast<OBSTNode<T>>(node->right),
                        level + 1, "    |");
      ...
      std::cout << node->key << " (p=" << std::fixed << std::setprecision(3)
                << node->probability << ")" << std::endl;

      print_tree_helper(std::dynamic_pointer_cast<OBSTNode<T>>(node->left),
                        level + 1, "    |");
    }
  }
};
...
void print_classic_example() {
  ...
  std::cout << "期望搜索成本: " << std::fixed << std::setprecision(3)
            << obst.get_expected_cost() << std::endl;
