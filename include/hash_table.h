...
public:
    // 除法散列法
    static int division_hash(int key, int table_size) {
...
public:
    PerfectHashTable(const std::vector<std::pair<K, V>>& elements) : total_elements(elements.size()) {
        if (elements.empty()) {
            primary_size = 1;
        ...
        if (primary_size == 0) primary_size = 1; // 确保主表大小至少为1
        primary_table.resize(primary_size, 0);
    ...
    // 搜索操作
    std::shared_ptr<V> search(K key) const {
        if (secondary_tables.empty()) return nullptr;
    ...
    void print() {
        ...
        for (int i = 0; i < primary_size && i < secondary_tables.size(); i++) {
            std::cout << "主槽 " << i << " -> 二级表大小: " 
                      << secondary_tables[i].size << std::endl;
        }
    }
};
...
public:
    // 测试散列函数分布
    static void test_hash_distribution(std::function<int(int)> hash_func, int table_size, int num_keys) {
        ...
        std::cout << "表大小: " << table_size << ", 键数量: " << num_keys << std::endl;
        ...
        std::cout << "平均槽元素数: " << static_cast<double>(num_keys) / table_size << std::endl;
    }
    
    // 性能比较
    template<typename HashTableType>
    static void performance_test(HashTableType& hash_table, const std::vector<std::pair<int, std::string>>& operations) {
        ...
        std::cout << "操作数量: " << operations.size() << ", 耗时: " << duration.count() << " 微秒" << std::endl;
    }
};
