...
public:
  ...
  // 判断节点是否已满
  bool is_full(int t) const { return n == 2 * t - 1; }

  // 判断节点是否过空
  bool is_minimum(int t) const { return n == t - 1; }
  ...
  void split_child(int i, int t) {
    ...
    // 如果不是叶子节点，移动子节点
    if (!y->is_leaf) {
      z->children.insert(z->children.begin(), y->children.begin() + t,
                         y->children.end());
    }
