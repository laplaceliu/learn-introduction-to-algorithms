...
struct ShortestPathResult {
  ...
  ShortestPathResult(int n)
      : distances(n, std::numeric_limits<int>::max()), predecessors(n, -1),
...
class BellmanFord {
public:
  static ShortestPathResult find_shortest_path(const AdjacencyListGraph &graph,
                                               int source) {
...
public:
  ...
  solve(const std::vector<std::tuple<int, int, int>> &constraints, int n) {
    // 构建约束图
