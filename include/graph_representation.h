...
struct GraphNode {
  ...
  bool operator==(const GraphNode &other) const { return id == other.id; }
...
void print_classic_examples() {
  ...
  std::cout << "是有向图: " << (undirected_graph.is_directed() ? "是" : "否")
            << std::endl;
  ...
  std::cout << "是有向图: " << (directed_graph.is_directed() ? "是" : "否")
            << std::endl;
}
