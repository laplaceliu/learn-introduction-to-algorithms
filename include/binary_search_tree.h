...
private:
    ...
    // 中序遍历辅助函数
    void inorder_helper(std::shared_ptr<BSTNode<T>> node, std::vector<T>& result) const {
        if (node != nullptr) {
    ...
    // 前序遍历辅助函数
    void preorder_helper(std::shared_ptr<BSTNode<T>> node, std::vector<T>& result) const {
        if (node != nullptr) {
            result.push_back(node->key);
    ...
    // 后序遍历辅助函数
    void postorder_helper(std::shared_ptr<BSTNode<T>> node, std::vector<T>& result) const {
        if (node != nullptr) {
    ...
    // 查找后继节点
    std::shared_ptr<BSTNode<T>> successor(std::shared_ptr<BSTNode<T>> node) const {
    ...
    // 查找前驱节点
    std::shared_ptr<BSTNode<T>> predecessor(std::shared_ptr<BSTNode<T>> node) const {
        if (node == nullptr) return nullptr;
    ...
    // 移植操作：用子树v替换子树u
    void transplant(std::shared_ptr<BSTNode<T>> u, std::shared_ptr<BSTNode<T>> v) {
        if (u == nullptr) return;
